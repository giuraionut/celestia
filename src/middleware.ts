...
const validateCommentPath = (pathname: string): boolean => {
  ...
  if (secondLastSegment === 'comments' && cuidSchema.safeParse(lastSegment).success) {
    return true;
  }
  return false;
};
...
export const config = {
  matcher: ['/api/auth/signup', '/api/auth/signin', '/api/auth/signout', '/api/auth/session', '/community/create', '/post/create', '/community/:path*', '/post/:path*'], // Apply middleware to these routes
};
